...
const TodoItem = ({ todo }: { todo: Todo }) => {
  ...
  return (
    <div
      className={twMerge(
        'flex flex-row items-center bg-slate-100 p-2',
        completed && 'bg-slate-200 line-through',
      )}
    >
      <label
        htmlFor={todo.id}
        className={twMerge(
          'block w-full select-none justify-center space-x-2 rounded',
          completed && 'bg-slate-200 line-through',
        )}
      >
      ...
      <button onClick={handleDownloadAttachment} className="h-6 w-6">
      ...
      <button onClick={handleDeleteTodo} className="h-6 w-6">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
